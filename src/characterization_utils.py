...
def load_file(fname, FSR=1000, bits=10,):
   '''
   ...
   #Initialize empty dict to store extracted data.
   filedata=dict()
   ...
   sorted_indices = np.argsort(filedata['time']) # Sorts based on time
   ...
   # Get height and baseline from each waveform
...
def get_remove_overflow_mask(energy_data, convert=False, FSR=1000, bits=10, digitizer_offset=148+8):
   '''
   energy_data - The energy data corresponding to the target data, used to identify overflow events
...
def landau_star_gauss(mu, c, x0, sigma, l, u, fast=False, grid_size=300, cdf=False):
   '''
   ...
   if cdf:
      if fast: interp1d(x_grid, pdf2cdf(x_grid, conv_values), kind='linear', bounds_error=False, fill_value=0) # For speed
      else: return BarycentricInterpolator(x_grid,pdf2cdf(x_grid,conv_values)) # Better Interpolation   
   if fast: return interp1d(x_grid, conv_values, kind='linear', bounds_error=False, fill_value=0) # For speed
   else: return BarycentricInterpolator(x_grid,conv_values) # Better Interpolation
...
#Sav:  hold up no this is now plotting. 
