...
def save_to_folder(outdir, root_fname, standard_results, fit_results):
   '''
   ...
   stan_pdf = FPDF()
   stan_pdf.add_page()
   stan_pdf.set_font("Times New Roman", size=12)
   fit_pdf = PdfPages(outdir / f'{root_fname}_results')
   ...
   with stan_pdf.table() as table:
      for data_row in standard_results:
         ...
         for datum in data_row:
            row.cell(datum)
   stan_pdf.output("standard_results.pdf")
   ...
   # Combine PDFs
   append_pdf(outdir, stan_pdf, fit_pdf)